...
def main() -> int:
    ...
    mask = 0xFF << 8
    with EziIOClient(
        args.host, timeout=0.5, allow_writes=True, output_offset=8, output_count=8
    ) as client:
        try:
            ...
            for channel in range(8):
                ...
                state_on = client.logical_outputs()
                ...
                state_off = client.logical_outputs()
            ...
            all_on = client.logical_outputs()
            print(f"ALL: ON  readback=0x{all_on:02X}")
            client.set_output(reset_mask=mask)
            all_off = client.logical_outputs()
            print(f"ALL: OFF readback=0x{all_off:02X}")
            if all_on != 0xFF or all_off != 0:
                raise RuntimeError("all-output readback mismatch")

            client.configure_trigger(0, period_ms=20, on_ms=10, count=3)
            client.run_channel_trigger(0, run=True)
            time.sleep(0.1)
            trigger_state = client.get_output()
            print(
                "TRIGGER0: 10ms ON / 20ms period / 3 count, "
                f"output=0x{(trigger_state.outputs >> 8) & 0xFF:02X}, "
                f"running=0x{(trigger_state.trigger_running >> 8) & 0xFF:02X}"
            )
        finally:
            client.run_channel_trigger(0, run=False)
            client.set_output(reset_mask=mask)
            final = client.get_output()
            print(f"SAFE OFF: output=0x{(final.outputs >> 8) & 0xFF:02X}")
    return 0
